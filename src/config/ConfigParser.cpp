...
{
	if (clean_file_str_.empty()) return;
								...
								const std::vector<std::string>& tokens)
{
	...
	if (pos != std::string::npos)
	{
		...
		else
		{
			...
			{
				throw ConfigException(
					config::errors::invalid_ip_format + ": " + first);
			}
	...
	else
	{
		...
		else
		{
			...
			{
				throw ConfigException(
					config::errors::invalid_ip_format + ": " + value);
			}
							...
							const std::vector<std::string>& tokens)
{
	...
	{
		throw ConfigException(
			config::errors::invalid_ip_format + ": " + hostValue);
	}
								...
								std::vector<std::string>& tokens)
{
	...
	// el mínimo son 3 tokens: error_page, 404 y /ruta)
	{
		...
		{
			server.addErrorPage(config::utils::stringToInt(tokens[i].c_str()),
								path);
		}
	}
}
 ...
 * upload_store /up\nloads;         X Newline
 */
void ConfigParser::parseUploadBonus(LocationConfig& loc,
									...
									std::vector<std::string>& tokens)
{
	...
	while (std::getline(ss, line))
	{
		...
		else if (directive == config::section::allow_methods)
		{
			...
			for (size_t i = 1; i < locTokens.size(); ++i)
			{
				std::string method = config::utils::removeSemicolon(
					locTokens[i]);
				if (method.empty()) continue;
				if (!config::utils::isValidHttpMethod(method))
				{
					throw ConfigException(
						config::errors::invalid_http_method + ": " +
						method);
				}
