...
inline std::ostream& operator<<(std::ostream& os, const LocationConfig& location) {
    os << config::colors::cyan << config::colors::bold << "Locations info:\n" << config::colors::reset
       << "\t" << config::colors::yellow << "Location Path: " << config::colors::reset << config::colors::green << location.getPath() << config::colors::reset << "\n"
       << "\t" << config::colors::yellow << "Root: " << config::colors::reset << config::colors::green << location.getRoot() << config::colors::reset << "\n"
	   << "\t" << config::colors::yellow << "Autoindex: " << config::colors::reset;
    ...
    if (!location.getUploadStore().empty()) {
        os << "\t" << config::colors::yellow << "Upload Store: " << config::colors::reset << config::colors::green << location.getUploadStore() << config::colors::reset << "\n";
    } else {
        os << "\t" << config::colors::yellow << "Upload Store: " << config::colors::reset << config::colors::red << "empty" << config::colors::reset << "\n";
    }

    if (location.getRedirectCode() != -1) {
        os << "\t" << config::colors::yellow << "Return (" << location.getRedirectParamCount() << " parameter" << (location.getRedirectParamCount() == 1 ? "" : "s") << "): " << config::colors::reset;
        if (location.getRedirectParamCount() == 1) {
            os << config::colors::green << location.getRedirectUrl() << config::colors::reset << "\n";
        } else {
            os << config::colors::green << location.getRedirectCode() << " '" << location.getRedirectUrl() << "'" << config::colors::reset << "\n";
        }
    }
    ...
    if (indexes.empty()) {
        ...
        for (size_t i = 0; i < indexes.size(); ++i) {
            os << config::colors::green << indexes[i] << config::colors::reset << (i == indexes.size() - 1 ? "" : ", ");
        }
    }

    // Imprimir Métodos
    const std::vector<std::string>& methods = location.getMethods();
    os << "\n\t" << config::colors::yellow << "Methods: " << config::colors::reset;
    if (methods.empty()) {
        os << config::colors::red << "empty" << config::colors::reset;
    } else {
        for (size_t i = 0; i < methods.size(); ++i) {
            os << config::colors::green << methods[i] << config::colors::reset << (i == methods.size() - 1 ? "" : ", ");
        }
    }
    os << "\n";
    return os;
