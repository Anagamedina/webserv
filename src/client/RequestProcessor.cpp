...
HttpResponse RequestProcessor::process(const HttpRequest& request,
  ...
  if (!location) {
    // Should rely on default location or root if not found, but usually there's
    // a "/" location. If no location matches, use server root. For now, let's
    // assume we always have a location or default to 404/403. Better: create a
    // default temporary location if none found? Or strictly strictly follow
    // Nginx: returns 404 if no match.
  ...
  if (stat(path.c_str(), &s) == 0) {
    if (s.st_mode & S_IFDIR) {
      // Check if it's a POST to a directory (meaning upload to this dir? or
      // CGI?)
      ...
      if (!ext.empty() && location->cgi_handlers.count(ext)) {
        ...
        CgiProcess* proc =
            cgi_executor_.executeAsync(request, path, interpreter);
        if (proc == NULL) {
...
RequestProcessor::findLocation(const std::string& uri,
                               const ServerBlock* config) {
  ...
  for (size_t i = 0; i < config->locations.size(); ++i) {
    ...
    if (uri.compare(0, path.length(), path) == 0) {
      ...
      // Needs to be robust: "/img" matches "/img/logo.png" but not
      // "/img_backup" So needs to match full segment or be equal

      if (path.length() > longuest_match) {
        // Verify segment boundary if not root
        if (path == "/" || uri.length() == path.length() ||
            uri[path.length()] == '/') {
...
std::string RequestProcessor::resolvePath(const std::string& uri,
  ...
  size_t question_mark = uri_suffix.find('?');
  if (question_mark != std::string::npos) {
    uri_suffix = uri_suffix.substr(0, question_mark);
  }
...
bool RequestProcessor::methodAllowed(const std::string& method,
                                     const LocationConfig& location) {
  if (location.accepted_methods.empty()) {
    return true; // Default allow all? Or default allow GET? usually GET is
                 // allowed.
  }
...
HttpResponse RequestProcessor::handleDirectory(const std::string& path,
                                               const LocationConfig& location) {
  // 1. Check Index
...
HttpResponse RequestProcessor::handleDelete(const std::string& path,
                                            const ServerBlock* config) {
...
HttpResponse RequestProcessor::handlePost(const HttpRequest& request,
...
HttpResponse RequestProcessor::handleError(int code,
                                           const ServerBlock* config) {
  ...
  // Default error page
  HttpResponse response(code);
  std::string body = "<html><body><h1>Error " + StringUtils::toString(code) +
                     "</h1></body></html>";
  response.setBody(body);
  response.setHeader("Content-Type", "text/html");
  return response;
...
CgiProcess* RequestProcessor::getCgiProcess() const { return cgi_process_; }

void RequestProcessor::clearCgiProcess() { cgi_process_ = NULL; }
