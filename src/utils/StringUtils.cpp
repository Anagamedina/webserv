...
#include <climits>
    ...
    // Since toString is a template in header, we don't strictly need intToString wrapping it unless for convenience.
    // But we declared stringToInt and stringToLong.
    
    int StringUtils::stringToInt(const std::string& str, int defaultValue) {
    ...
    long StringUtils::stringToLong(const std::string& str, long defaultValue) {
        if (str.empty()) return defaultValue;
