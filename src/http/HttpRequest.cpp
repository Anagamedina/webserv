...
/**
 * @brief Agrega un header al mapa. La clave debe venir ya normalizada en
 * minúsculas.
 *
...
HttpVersion HttpRequest::getVersion() const { return _version; }

/**
 * @brief Obtiene un header específico de la petición
 *
 * @param key : es el nombre del header ej: "Host", "Content-Length",
 * "Connection"
 * @return const std::string& : es el contenido del header ej: "localhost:8080"
 * o "100000" o "keep-alive"
 *
 */
...
const std::string& HttpRequest::getHeader(const std::string& key) const {
  ...
  std::string lowerKey;
  // static porque se crea una sola vez y se reutiliza en todas las llamadas a
  // la función
  ...
  if (it != _headers.end()) return it->second;  // value lo que nos interesa
 ...
 * y util para CGI que necesita iterar sobre todos los headers para variables de
 * entorno.  Carles
 */
...
/**
 * @brief Limpia el request porque se puede reutilizar el objeto para otra
 * petición y se debe limpiar el estado del request para que no haya
 * interferencias con la siguiente petición
 *
...
/**
 * @brief Verifica si se debe cerrar la conexión con el cliente después de
 * enviar la respuesta
 *
