...
void HttpParser::parseBodyFixedLength()
{
    ...
    if (toRead > 0)
    {
       ...
       //necesito saber cuantos bytes del body he llevo acumulados para saber si he leido todo el body.
...
bool HttpParser::parseChunkSizeLine(std::size_t &size)
{
    ...
    //OJO! El estándar HTTP (RFC 9112) dice explícitamente: "Un receptor DEBE ignorar las extensiones de chunk que no comprenda
    // Ignorar extensiones (ej: "1a;ext=foo")
    semi = line.find(';');
    if (semi != std::string::npos)
        line = line.substr(0, semi);
//strtoul mueve el puntero endPtr hasta donde dejó de leer. 
//Si endPtr se queda al principio del string (line.c_str()), significa que no pudo leer ni un solo número válido.
...
bool HttpParser::handleChunkSizeState()
{
    ...
    _chunkSize = size;
    // OJO! En el protocolo HTTP Chunked, el final del mensaje se indica siempre con un bloque de tamaño 0.
    if (_chunkSize == 0)
    {
